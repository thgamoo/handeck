/**
 * ⚠️ **옛 생성기다. 지금은 돌리지 말 것 (2026-08-14).**
 *
 * `example/project.json` 은 이제 **앱에서 실제로 그린 작업본**(`samaksil.handeck`)을
 * 풀어 넣은 것이다. 이 스크립트를 돌리면 그 위에 «자리만 있는» 프로젝트를 덮어쓴다.
 * 규칙 문서의 표를 코드로 옮기는 방식이 다시 필요해질 때 참고하라고 남겨둔다.
 *
